...
const ICONS = {
  receipt: "M6 2h12v20l-2-1.4-2 1.4-2-1.4-2 1.4-2-1.4L6 22zM9 8h6M9 12h6M9 16h3",
...
const FEATURES = [
  {
    ...
    d: "True thermal output at 58mm, 80mm, 110mm and A4 — correct column widths, torn edges and monospace print. Not a table pretending to be a receipt.",
  },
  {
    icon: ICONS.calc,
    t: "The maths always adds up",
    d: "Subtotal, discount, tax and tip calculate in the right order and update live as you type. The arithmetic on the page always reconciles.",
  },
  {
    icon: ICONS.globe,
  ...
  },
  {
    icon: ICONS.image,
    t: "Your own logo",
    d: "Upload your business logo and it prints cleanly in the header. Your branding, on your receipts.",
  },
  {
    icon: ICONS.download,
    t: "PNG, PDF & JPEG",
...
const STEPS = [
  ...
  },
  {
    ...
    d: "Export a print-ready PNG, PDF or JPEG in one click. No signup, no forced watermark, no waiting.",
  },
];
  ...
  return (
    <>
      ...
      <section className="border-b border-[var(--line)] bg-white">
        <div className="container-x grid gap-14 py-16 lg:grid-cols-[1fr_1.05fr] lg:items-center lg:py-24">
          <div>
            <p className="eyebrow mb-5">Free · No signup · 60 seconds</p>

            <h1 className="text-balance font-display text-[clamp(2.6rem,5.6vw,4.1rem)] font-extrabold leading-[1.02] tracking-[-0.035em] text-ink-50">
              The better way to make a receipt
            </h1>

            <p className="mt-6 max-w-[50ch] text-[17px] leading-relaxed text-ink-300 text-pretty">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-2 text-[13.5px] text-ink-400">
              {["No signup", "No forced watermark", "Free forever", "PNG · PDF · JPEG"].map(
                (x) => (
                  <span key={x} className="inline-flex items-center gap-1.5">
                    <Icon path={ICONS.check} className="h-3.5 w-3.5 text-[var(--accent)]" />
                    {x}
                  </span>
                )
              )}
      ...
      {/* ══════════════════ trust stats ══════════════════ */}
      <section className="border-b border-[var(--line)] bg-white">
        <div className="container-x grid grid-cols-2 gap-y-8 py-10 sm:grid-cols-3 lg:grid-cols-5">
          {[
            ...
            <div
              key={label}
              className={`px-4 text-center lg:border-l lg:border-[var(--line)] lg:first:border-l-0 ${
                i === 4 ? "col-span-2 sm:col-span-1" : ""
              }`}
            >
              <div className="font-display text-[clamp(1.5rem,2.6vw,2rem)] font-extrabold tracking-[-0.03em] text-ink-50">
                {num}
              </div>
              <div className="mt-1 text-[12px] font-medium uppercase tracking-[0.08em] text-ink-400">
                {label}
              </div>
            </div>
          ))}
      ...
      <section className="container-x py-24">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="font-display text-[clamp(1.9rem,3.6vw,2.6rem)] font-bold leading-[1.1] tracking-[-0.03em] text-ink-50">
            Make a receipt in three steps
          </h2>
        ...
        <ol className="mt-14 grid gap-5 md:grid-cols-3">
          {STEPS.map((s, i) => (
            <li key={s.t} className="card-soft p-8">
              <div className="flex items-center justify-between">
                <span className="grid h-10 w-10 place-items-center rounded-[8px] border border-[var(--line)] bg-white text-ink-50">
                  <Icon path={s.icon} className="h-5 w-5" />
                </span>
                <span className="font-display text-2xl font-bold tracking-[-0.03em] text-ink-500">
                  0{i + 1}
                </span>
              </div>
        ...
        <div className="container-x py-24">
          <div className="mb-12 flex flex-wrap items-end justify-between gap-4">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 max-w-[54ch] text-[16px] leading-relaxed text-ink-300">
                Every template is a real, itemised receipt — logo, line items, tax and totals
          ...
          <div className="grid grid-cols-2 items-stretch gap-4 sm:gap-5 lg:grid-cols-4">
            {featured.map((t) => (
              <Link
                ...
                className="group flex h-full flex-col rounded-[12px] border border-[var(--line)] bg-white px-4 pb-5 pt-8 transition-all duration-200 hover:-translate-y-1 hover:shadow-[var(--shadow-2)]"
              >
                ...
                <div className="mt-6 text-center">
                  ...
                  <h3 className="mt-1 font-display text-[15px] font-bold tracking-[-0.01em] text-ink-50 transition-colors group-hover:text-[var(--accent)]">
                    {t.name}
                  </h3>
                </div>
              </Link>
      ...
      <section className="container-x py-24">
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f) => (
            <div key={f.t} className="card-soft p-7">
              <span className="grid h-10 w-10 place-items-center rounded-[8px] border border-[var(--line)] bg-white text-ink-50">
              ...
              <p className="mt-2 text-[13.5px] leading-relaxed text-ink-300">{f.d}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ══════════════════ why choose us ══════════════════ */}
      <section className="border-y border-[var(--line)] bg-[var(--paper-2)]">
        <div className="container-x grid gap-12 py-24 lg:grid-cols-[1fr_1.1fr] lg:items-center">
          <div>
            <p className="eyebrow mb-4">Why BuildMyReceipt</p>
            <h2 className="font-display text-[clamp(1.9rem,3.6vw,2.6rem)] font-bold leading-[1.1] tracking-[-0.03em] text-ink-50">
              The receipt maker people keep coming back to
            </h2>
            <p className="mt-4 max-w-[50ch] text-[16px] leading-relaxed text-ink-300">
              Most receipt tools hide the good part behind a signup, force a watermark you
          ...
          <ul className="grid gap-x-6 gap-y-4 sm:grid-cols-2">
            {WHY.map((w) => (
              <li key={w} className="flex items-start gap-3 text-[14.5px] text-ink-200">
          ...
          <div className="divide-y divide-[var(--line)] border-y border-[var(--line)]">
            {FAQS.map((f) => (
              <details key={f.q} className="group">
                ...
                <p className="pb-5 pr-8 text-[14px] leading-relaxed text-ink-300">{f.a}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section className="container-x pb-8">
        <div className="card-soft px-8 py-16 text-center">
          <h2 className="font-display text-[clamp(1.8rem,3.4vw,2.5rem)] font-extrabold tracking-[-0.03em] text-ink-50">
            Make your first receipt now
          </h2>
          <p className="mx-auto mt-3 max-w-md text-[16px] leading-relaxed text-ink-300">
            Free, no account, no card. Open the receipt maker and it just works.
          </p>
          <Link href="/generator" className="btn btn-primary btn-lg mt-8">
      ...
      {/* ══════════════════ seo copy ══════════════════ */}
      <section className="container-x pb-24 pt-16">
